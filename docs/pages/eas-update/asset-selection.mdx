...
> **info** Asset selection controls which assets are eligible for over-the-air updates. It does not change which assets are bundled into the native binary, so it does not reduce app startup time. On iOS, embedded assets are served directly from the app binary on first launch and are not copied into the updates cache by default.
> If you want embedded assets pre-seeded into the cache so a later update that reuses them avoids a re-download, set `updates.copyEmbeddedAssets` to `true`. Note that this runs the copy while the app launches and can slow first launch for apps with many bundled assets.
